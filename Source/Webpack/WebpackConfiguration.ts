...
export class WebpackConfiguration {
    ...
    constructor(private _rootDir: string, private _environment: WebpackEnvironment, private _argv: WebpackArguments) {}
    ...
    private getEntry() {
        return fs.existsSync(path.resolve(this._featuresDir, 'main.ts'))? path.resolve(this._featuresDir, 'main.ts') : path.resolve(this._featuresDir, 'main.js');
    }

    private getOutput() {
        let output: Output = {
            filename: this._environment.production === true? '[name].[chunkhash].bundle.js' : '[name].[hash].bundle.js',
            sourceMapFilename: this._environment.production === true ? '[name].[chunkhash].bundle.map' : '[name].[hash].bundle.map',
            chunkFilename: this._environment.production === true ? '[name].[chunkhash].chunk.js' : '[name].[hash].chunk.js',
            path: this._outDir,
        ...
        let module: Module = {
            rules: [
                ...
                { test: /environment\.json$/i, use: [
                    {loader: "app-settings-loader", options: {env: this._environment.production === true? 'production' : 'development' }},
                ]},
            ]
        };
        return module;
            ...
            new HtmlWebpackPlugin({
                ...
                template: fs.existsSync(path.resolve(this._rootDir, 'index.ejs'))?
                    ...
                    metadata: {
                        ...
                        title: this._title, baseUrl: this._baseUrl
                    }
            }),
            ...when(this._argv.extractCss === true, new MiniCssExtractPlugin({ // updated to match the naming conventions for the js files
